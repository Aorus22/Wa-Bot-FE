...
import { Send, Smile, Paperclip, FileText, Video, Sticker, Star, Trash2, ArrowLeft, MessageSquare, Reply, Edit3, X, MoreVertical, Image, Link, File } from "lucide-react"
...
const MessageItem = memo(({ 
	...
	const handleTouchMove = (e: React.TouchEvent) => {
		...
		// Only swipe from left to right for reply (standard WA)
		if (diff > 0) {
													...
													{isSticker ? (
														...
														>
															{isImage && isMedia && (
																<div className="mb-2 -mx-1 -mt-1 rounded-lg overflow-hidden border border-black/5 dark:border-white/5">
																	<img 
																		...
																		alt="Image" 
																		className="w-full max-w-[320px] h-auto object-cover hover:scale-[1.02] transition-transform duration-500 cursor-zoom-in" 
																		onClick={() => setSelectedImageUrl(getMediaUrl(message.mediaUrl) || null)}
																	/>
																</div>
															)}
															{isVideo && isMedia && (
															...
															<div className={cn(
																...
																{/* Desktop Context Menu & Mobile Long Press Support */}
																<Popover>
																	<PopoverTrigger asChild>
																		<div 
																			className="absolute inset-0 z-0 cursor-context-menu"
																		...
																		<div className="flex flex-col">
																			...
																			{isMe && (
																				<>
																					...
																					<button
																						onClick={() => handleDeleteMessage(message.id)}
																						className="flex items-center gap-3 w-full p-2.5 hover:bg-muted rounded-lg text-sm transition-colors text-destructive"
																					>
																						<Trash2 className="h-4 w-4" />
												...
												<div key={m.id} className="aspect-square rounded-lg overflow-hidden border border-border/40 bg-muted/20 group relative">
													{m.type === "image" ? (
														<img 
															...
															onClick={() => setSelectedImageUrl(getMediaUrl(m.mediaUrl) || null)}
														/>
													) : (
														<div 
