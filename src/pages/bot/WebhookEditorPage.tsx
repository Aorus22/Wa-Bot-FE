...
import { ArrowLeft, Save, Play, CheckCircle2, X, Sparkles, Loader2, Pencil, FileCode, Eye, EyeOff } from 'lucide-react'
...
export function WebhookEditorPage() {
	...
	const webhookFromNav = (location.state as any)?.webhook as Webhook | undefined
	...
	useEffect(() => {
		if (webhookFromNav) setFormData(webhookFromNav)
	}, [webhookFromNav])
	...
	const handleTest = async () => {
		try {
			...
			if (!formData.path || !formData.script) {
				toast.error('Webhook path and script are required')
				return
			}
			setIsTesting(true)
			setConsoleLogs(prev => [...prev, {
				time: formatTimestamp(new Date()),
				type: 'invoke',
				message: `Starting execution for path: /webhook/${formData.path} (${testMethod})`
			}])
			...
			if (result.error) {
				...
				if (result.response) {
					setConsoleLogs(prev => [...prev, {
						time: formatTimestamp(new Date()),
						type: 'print',
						message: `Response: ${JSON.stringify(result.response, null, 2)}`
					}])
				}
			}
		} catch (error: any) {
		...
		<div className="h-full w-full bg-background flex flex-col min-h-0">
				...
				<div className="flex items-start gap-3 min-w-0">
					...
					<div className="min-w-0">
						<h1 className="text-xl md:text-2xl font-semibold tracking-tight truncate">
							{formData.name || 'Untitled Webhook'}
						</h1>
				...
				<div className="mx-auto w-full max-w-6xl px-4 md:px-8 py-6 md:py-8">
					...
					<Card className="mb-6">
						<CardHeader className="border-b border-border/40 pb-5">
							...
							<CardDescription>Basic configuration for your webhook endpoint.</CardDescription>
						</CardHeader>
						<CardContent className="space-y-5">
								...
								<div className="space-y-2">
									...
									<div className="relative">
										...
										<Input
											...
											className="h-10 font-mono pl-[5.5rem]"
										/>
									</div>
								</div>
							</div>
							<div className="space-y-2">
								...
								<Textarea
									placeholder="Briefly describe what this webhook does..."
			...
			>
				...
				<div className="flex flex-1 overflow-hidden">
					{/* Left Pane: Method + Payload */}
					<div className="flex flex-col flex-[0.8] border-r border-[#2a2a2a] min-w-0">
						<div className="flex items-center gap-3 px-4 py-2 bg-[#1a1a1a] border-b border-[#2a2a2a] shrink-0">
							<Select value={testMethod} onValueChange={setTestMethod}>
								<SelectTrigger className="h-7 px-2.5 py-0 text-[11px] font-mono w-[90px] bg-[#252525] border-[#444] text-[#ccc] rounded-md shadow-none">
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
							...
							<span className="text-[11px] font-mono uppercase tracking-[0.05em] text-[#888]">
								Payload (JSON)
							</span>
						</div>
